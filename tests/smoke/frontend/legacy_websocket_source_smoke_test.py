...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the legacy WebSocket browser smoke against a managed source server."
    )
    ...
    with ManagedSourceServer(
        port=args.port,
        no_katago=True,
        artifact_subdir="legacy-websocket-source-smoke",
        ...
        result = subprocess.run(
            [
                npm,
                "run",
                "smoke:legacy-websocket-messages",
                "--prefix",
                "frontend",
                "--",
