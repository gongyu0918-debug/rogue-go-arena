...
function translateServerEventMessageLegacy(message, force = false) {
  ...
  const patterns = [
    ...
    [/^⚡ 快速思考：([0-9.]+) 秒追加手已开启$/, (seconds) => `⚡ Quick Thinking: the ${seconds}-second bonus move is now active`],
...
function translateServerEventMessageWestern(message, lang) {
  ...
  const patterns = [
    ...
    [/^⚡ 快速思考：([0-9.]+) 秒追加手已开启$/, (seconds) => pick(`⚡ Reflexion rapide : fenetre de ${seconds} s ouverte pour un coup bonus.`, `⚡ Schnelles Denken: ${seconds}-s-Fenster fuer einen Bonuszug ist offen.`)],
...
function translateServerEventMessage(message) {
  ...
  const patterns = [
    ...
    [/^⚡ 快速思考：([0-9.]+) 秒追加手已开启$/, (seconds) => pick(`⚡ クイック思考：${seconds}秒の追加手が有効になりました`, `⚡ 빠른 사고: ${seconds}초 추가 착수가 활성화되었습니다`)],
