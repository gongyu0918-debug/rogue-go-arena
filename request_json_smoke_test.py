...
async def smoke_card_config_route_uses_shared_json_reader() -> None:
    ...
    try:
        ...
        saved = await s.save_card_config_payload(FakeRequest({"config": {"cards": {}}}))
        assert saved == {"ok": True, "saved": {"cards": {}}}
        assert calls == [{"cards": {}}]

        saved_from_list = await s.save_card_config_payload(FakeRequest(["not", "an", "object"]))
        assert saved_from_list == {"ok": True, "saved": None}
        assert calls[-1] is None

        saved_from_null = await s.save_card_config_payload(FakeRequest(None))
        assert saved_from_null == {"ok": True, "saved": None}
        assert calls[-1] is None

        invalid = await s.save_card_config_payload(FakeRequest(raises=True))
...
async def smoke_balance_route_preserves_non_object_body_behavior() -> None:
    ...
    try:
        ...
        saved = await s.save_balance_lab_payload(FakeRequest({"values": {"ROGUE_DICE_PASS_CHANCE": 1.0}}))
        assert saved == {"ok": True, "values": {"ROGUE_DICE_PASS_CHANCE": 1.0}}
        assert calls[-1] == {"ROGUE_DICE_PASS_CHANCE": 1.0}

        saved_from_list = await s.save_balance_lab_payload(FakeRequest(["not", "an", "object"]))
        assert saved_from_list == {"ok": True, "values": {}}
        assert calls[-1] == {}

        invalid = await s.save_balance_lab_payload(FakeRequest(raises=True))
        assert invalid.status_code == 400
        assert response_payload(invalid) == {
            "ok": False,
            "errors": ["request body must be JSON"],
