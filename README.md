<p align="center">
  <img src="rogue-go-arena.png" width="92" alt="rogue-go-arena icon">
</p>
...
| --- | --- |
| <img src="docs/assets/rogue-go-arena-hero-current.jpg" alt="Rogue Go Arena current board with winrate and wood UI"> | <img src="docs/assets/rogue-go-arena-rogue-guide.jpg" alt="Rogue Go Arena wiki guide panel"> |

| Rogue Card Draft |
| --- |
| <img src="docs/assets/rogue-go-arena-card-draft.jpg" alt="Rogue Go Arena card selection screen"> |
...
```text
rogue-go-arena.exe
  starts/reuses rogue-go-arena-server.exe
...
完整明文配置见 [app/data/cards.json](app/data/cards.json)，结构约束见 [app/data/cards.schema.json](app/data/cards.schema.json)。游戏每次开局都会重新加载本机配置；可视化编辑器保存到 `%LOCALAPPDATA%\rogue-go-arena\cards.json`，保留基准配置作为回退。
...
```powershell
python -m PyInstaller --clean --noconfirm rogue-go-arena.spec
```

构建服务端：

```powershell
python -m PyInstaller --clean --noconfirm rogue-go-arena-server.spec
```
...
`-PythonExe "C:\path\to\python.exe"` 明确指定构建解释器。
